    ...
    public void incrementFirstPlayer(View view) {
        ...
        if (gameScoreFirstPlayer >= pointsAmount && gameScoreFirstPlayer - gameScoreSecondPlayer > 1) {
            ...
            if (matchScoreFirstPlayer == gamesAmount/2 + 1) {
                Toast.makeText(this, currentgame.getFirstPlayerName() + " won. Congratulations!", Toast.LENGTH_SHORT).show();
                resetAll(view);
            } else {
                Toast.makeText(this, currentgame.getFirstPlayerName() + " won this game", Toast.LENGTH_SHORT).show();
                resetScore(view);
            }
            return;
        }
    ...
    public void incrementSecondPlayer(View view) {
        ...
        if (gameScoreSecondPlayer >= pointsAmount && gameScoreSecondPlayer - gameScoreFirstPlayer > 1) {
            ...
            if (matchScoreSecondPlayer == gamesAmount/2 + 1) {
                Toast.makeText(this, currentgame.getSecondPlayerName() + " won. Congratulations!", Toast.LENGTH_SHORT).show();
                resetAll(view);
            } else {
                Toast.makeText(this, currentgame.getSecondPlayerName() + " won this game", Toast.LENGTH_SHORT).show();
                resetScore(view);
            }
            return;
        }
